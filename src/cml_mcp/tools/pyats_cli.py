...
def pyats_send_cli_command_sync(
    ...
    pyats_username = _pyats_username.get() or os.getenv("PYATS_USERNAME")
    pyats_password = _pyats_password.get() or os.getenv("PYATS_PASSWORD")
    pyats_auth_pass = _pyats_auth_pass.get() or os.getenv("PYATS_AUTH_PASS")
    for device in testbed.devices.values():
        if device.name != "terminal_server":
            ...
            device.credentials.enable.password = pyats_auth_pass or device.credentials.default.password
