...
- **Native CLI API** — `send_cli_command` uses `POST /labs/{id}/nodes/{id}/cli`
  on CML 2.11+ (no `virl2_client`, PyATS, or Unicon on the internal wheel).
- **`pyats_cli.py` optional fallback** — kept for parity with upstream when
  connecting to controllers older than 2.11; inactive on-controller where CML
  is always 2.11+.
- **No `virl2_client` dependency** — `cml_client.py` is httpx-only; CLI uses
...
The old `unicon_cli.py` / `termws` path is obsolete after CMLDEV-761; do not
reintroduce it on current release lines.
