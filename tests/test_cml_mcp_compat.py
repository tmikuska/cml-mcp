"""
Live MCP tool coverage against a CML 2.11 controller.

    pytest --controller-url=https://cml.example.com packaging/mcp_server/tests/test_cml_mcp_compat.py
"""
...
async def test_node_definitions(main_mcp_client: Client[FastMCPTransport]):
    ...
    detail = await main_mcp_client.call_tool(name="get_node_definition_detail", arguments={"definition_id": did})
...
async def test_node_lifecycle(main_mcp_client: Client[FastMCPTransport], compat_lab):
    ...
    cfg = await main_mcp_client.call_tool(
        ...
        arguments={"lab_id": compat_lab, "node_id": nid, "config": "hostname compat-test"},
    )
    assert cfg.data is True

    start = await main_mcp_client.call_tool(
        name="start_cml_node",
        arguments={"lab_id": compat_lab, "node_id": nid, "wait_for_convergence": True},
    )
...
async def test_annotation_crud(main_mcp_client: Client[FastMCPTransport], compat_lab):
    """Tools: add ellipse/rectangle/text annotations, get_annotations, delete_annotation."""
...
async def test_pcap_and_cli(main_mcp_client: Client[FastMCPTransport]):
    ...
    try:
        ...
        intf1 = await main_mcp_client.call_tool(name="get_interfaces_for_node", arguments={"lab_id": lab_id, "node_id": n1_id})
        intf2 = await main_mcp_client.call_tool(name="get_interfaces_for_node", arguments={"lab_id": lab_id, "node_id": n2_id})
        ...
        try:
            ...
            if "failed to connect" in str(exc):
                pytest.skip(f"Node not reachable for native /cli: {exc}")
            raise
