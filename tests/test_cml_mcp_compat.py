...
Run manually against both 2.9 and 2.10 to verify compatibility:
    ...
    pytest --controller-url=https://2.9.server:port packaging/mcp_server/tests/test_cml_mcp_compat.py
    pytest --controller-url=https://2.10.server:port packaging/mcp_server/tests/test_cml_mcp_compat.py
"""
...
async def test_user_tools(main_mcp_client: Client[FastMCPTransport]):
    ...
    result = await main_mcp_client.call_tool(
        ...
        arguments={"username": "mcp_compat_user", "password": "CompatTest123!"},
    )
...
async def test_group_tools(main_mcp_client: Client[FastMCPTransport]):
    ...
    result = await main_mcp_client.call_tool(
        name="create_cml_group",
        arguments={"name": "mcp_compat_group", "description": "Compat test group"},
    )
...
async def test_node_definitions(main_mcp_client: Client[FastMCPTransport]):
    ...
    definition_id = first_def["id"] if isinstance(first_def, dict) else first_def.id
    detail = await main_mcp_client.call_tool(
        name="get_node_definition_detail",
        arguments={"definition_id": definition_id},
    )
...
async def compat_lab(main_mcp_client: Client[FastMCPTransport]):
    ...
    result = await main_mcp_client.call_tool(
        ...
        arguments={"title": "MCP Compat Lab", "description": "Compatibility test lab"},
    )
...
async def test_modify_lab(main_mcp_client: Client[FastMCPTransport], compat_lab):
    ...
    result = await main_mcp_client.call_tool(
        name="modify_cml_lab",
        arguments={"lab_id": compat_lab, "title": "MCP Compat Lab Modified"},
    )
    assert result.data is True
    await main_mcp_client.call_tool(
        name="modify_cml_lab",
        arguments={"lab_id": compat_lab, "title": "MCP Compat Lab"},
    )


async def test_download_topology(main_mcp_client: Client[FastMCPTransport], compat_lab):
    """Tool: download_lab_topology."""
    result = await main_mcp_client.call_tool(name="download_lab_topology", arguments={"lab_id": compat_lab})
...
async def test_node_lifecycle(main_mcp_client: Client[FastMCPTransport], compat_lab):
    ...
    node_id = _extract_uuid(result)

    nodes = await main_mcp_client.call_tool(name="get_nodes_for_cml_lab", arguments={"lab_id": compat_lab})
    assert isinstance(nodes.data, list)
    assert len(nodes.data) >= 1

    cfg = await main_mcp_client.call_tool(
        name="configure_cml_node",
        arguments={"lab_id": compat_lab, "node_id": node_id, "config": "hostname compat-test"},
    )
    assert cfg.data is True

    start = await main_mcp_client.call_tool(
        name="start_cml_node",
        arguments={"lab_id": compat_lab, "node_id": node_id, "wait_for_convergence": True},
    )
    assert start.data is True

    stop = await main_mcp_client.call_tool(
        name="stop_cml_node",
        arguments={"lab_id": compat_lab, "node_id": node_id},
    )
...
async def test_interface_and_link_mgmt(main_mcp_client: Client[FastMCPTransport], compat_lab):
    ...
    n1_result = await main_mcp_client.call_tool(
        name="add_node_to_cml_lab",
        arguments={
            "lab_id": compat_lab,
            "node_definition": "alpine",
    ...
    n2_result = await main_mcp_client.call_tool(
        name="add_node_to_cml_lab",
        arguments={
            "lab_id": compat_lab,
            "node_definition": "alpine",
    ...
    intf_result = await main_mcp_client.call_tool(
        ...
        arguments={"lab_id": compat_lab, "node": n1_id},
    )
    ...
    intf1 = await main_mcp_client.call_tool(
        ...
        arguments={"lab_id": compat_lab, "node_id": n1_id},
    )
    ...
    intf2 = await main_mcp_client.call_tool(
        name="get_interfaces_for_node",
        arguments={"lab_id": compat_lab, "node_id": n2_id},
    )
    ...
    link_result = await main_mcp_client.call_tool(
        ...
        arguments={"lab_id": compat_lab, "src_int": src_intf_id, "dst_int": dst_intf_id},
    )
    link_id = _extract_uuid(link_result)

    links = await main_mcp_client.call_tool(name="get_all_links_for_lab", arguments={"lab_id": compat_lab})
    ...
    await main_mcp_client.call_tool(
        name="start_cml_lab",
        arguments={"lab_id": compat_lab, "wait_for_convergence": True},
    )

    start_link = await main_mcp_client.call_tool(
        name="start_cml_link",
        arguments={"lab_id": compat_lab, "link_id": link_id},
    )
...
# Annotations
# ---------------------------------------------------------------------------
# Tools: add_ellipse_annotation, add_rectangle_annotation, add_text_annotation,
#        get_annotations_for_cml_lab, delete_annotation_from_lab


async def test_annotation_crud(main_mcp_client: Client[FastMCPTransport], compat_lab):
    """Tools: add annotations, get_annotations, delete_annotation."""
...
async def test_pcap_and_cli(main_mcp_client: Client[FastMCPTransport]):
    ...
    then tears down.
    """
    lab_result = await main_mcp_client.call_tool(
        name="create_empty_lab",
        arguments={"title": "MCP Compat PCAP Lab", "description": "PCAP and CLI compat test"},
    )
    ...
    try:
        ...
        intf1 = await main_mcp_client.call_tool(
            name="get_interfaces_for_node",
            arguments={"lab_id": lab_id, "node_id": n1_id},
        )
        intf2 = await main_mcp_client.call_tool(
            name="get_interfaces_for_node",
            arguments={"lab_id": lab_id, "node_id": n2_id},
        )
        ...
        link_result = await main_mcp_client.call_tool(
            name="connect_two_nodes",
            arguments={"lab_id": lab_id, "src_int": src_id, "dst_int": dst_id},
        )
        _extract_uuid(link_result)
        ...
        try:
            cli = await main_mcp_client.call_tool(
                ...
                arguments={"lab_id": lab_id, "label": "PCAP Node 1", "commands": "ping 192.0.2.2"},
            )
        ...
        links = await main_mcp_client.call_tool(name="get_all_links_for_lab", arguments={"lab_id": lab_id})
        ...
        pcap_start = await main_mcp_client.call_tool(
            ...
            arguments={"lab_id": lab_id, "link_id": actual_link_id, "maxpackets": 50, "bpfilter": "icmp"},
        )
        ...
        pcap_status = await main_mcp_client.call_tool(
            ...
            arguments={"lab_id": lab_id, "link_id": actual_link_id},
        )
...
async def test_lab_stop_wipe_delete(main_mcp_client: Client[FastMCPTransport]):
    ...
    lab_result = await main_mcp_client.call_tool(
        name="create_empty_lab",
        arguments={"title": "MCP Compat Lifecycle Lab", "description": "Lab lifecycle test"},
    )
    lab_id = _extract_uuid(lab_result)
    ...
    try:
        ...
        stop = await main_mcp_client.call_tool(name="stop_cml_lab", arguments={"lab_id": lab_id})
        assert stop.data is True
