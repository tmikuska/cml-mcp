...
from cml_mcp.cml.simple_webserver.schemas.system import (
    SystemHealth,
    SystemInformation,
    SystemStats
)
...
COMMON_TEST_LAB_TITLE = _constants.COMMON_TEST_LAB_TITLE
...
async def _is_ping_reachable(
    ...
    pytest.fail(
        f"Ping to {target} from {label} did not succeed within {timeout}s; "
        f"last output: {last_out!r}"
    )
...
async def test_user_mgmt(main_mcp_client: Client[FastMCPTransport]):
    result = await main_mcp_client.call_tool(
        ...
        arguments={"username": "mcp_test_user", "password": "TestPassword123!"},
    )
    ...
    result = await main_mcp_client.call_tool(
        name="create_cml_user",
        arguments={"username": "mcp_test_user2", "password": "TestPassword123!"},
    )
    assert isinstance(result.content, list)
    assert len(result.content) > 0
    assert isinstance(result.content[0], TextContent)
...
async def test_get_cml_groups(main_mcp_client: Client[FastMCPTransport]):
    ...
    _ = await main_mcp_client.call_tool(
        ...
        arguments={"name": "mcp_test_group", "description": "Test group created by MCP tests"},
    )
...
async def test_group_mgmt(main_mcp_client: Client[FastMCPTransport]):
    ...
    result = await main_mcp_client.call_tool(
        name="create_cml_group",
        arguments={"name": "mcp_test_group2", "description": "Test group created by MCP tests"},
    )
    assert isinstance(result.content, list)
    assert len(result.content) > 0
    assert isinstance(result.content[0], TextContent)
...
async def test_get_annotations_for_cml_lab(main_mcp_client: Client[FastMCPTransport]):
    ...
    for annotation in ann_result.data:
        ...
        if isinstance(annotation, dict):
            ...
            # MCP returns canonical border_style; do not re-validate with webserver
            # *AnnotationResponse models (they expect legacy "", "2,2", "4,2").
            if ann_type == "ellipse":
...
async def test_add_annotation_to_cml_lab(main_mcp_client: Client[FastMCPTransport], created_lab: UUID4Type):
    ...
    for annotation in ann_result.data:
        if isinstance(annotation, dict):
            ann_type = annotation.get("type")
            assert ann_type in {"ellipse", "line", "rectangle", "text"}
...
@pytest.mark.live_only
async def test_cml_api_accepts_canonical_border_style_direct(
    live_cml_api_client, created_lab: UUID4Type
):
